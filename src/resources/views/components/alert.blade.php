...
>
    <div class="{{ $iconWrapClasses }}">
        <x-i :type="$iconType" variant="linear" class="w-[1.25em] h-[1.25em]" />
    </div>

    <div class="flex-1">
        @if($title)
            <p class="font-semibold {{ $titleTextClass }}">{{ $title }}</p>
        @endif
        <div class="{{ $title ? 'mt-1' : '' }} {{ $bodyTextClass }}">
            {{ $slot }}
        </div>
    </div>
    ...
    @if($dismissible)
        <button
            ...
            aria-label="Close"
        >
            {{-- Plain X close glyph via solar-extra (sibling library to Solar
                 that ships the shape-less close / check / plus / minus
                 variants Solar itself doesn't). --}}
            <x-i type="close" library="solar-extra" variant="linear" class="w-[1em] h-[1em]" />
        </button>
    @endif
</div>
