...
| **Shape · border · size · _rhythmic_ spacing** (must follow the tune) | pinion-ui tune classes & tune tokens — `tune-border`; `tune-btn-{xs,sm,md,lg}` / `tune-input-*` / `tune-textarea-*` / `tune-card-pad`; **spacing utilities** `space-section` · `space-section-inner`/`gap-section-inner` · `gap-element`/`mt-element`/`mb-element`/`p-element`/`space-y-element` · `gap-compact`/`p-compact`/`px-compact`/`py-compact`/`space-y-compact` · `gap-text`/`mt-text`/`mb-text`/`space-y-text` · `gap-inline`/`space-x-inline` · `gap-micro`/`space-x-micro`/`space-y-micro`; raw tokens `rounded-[var(--radius-box)]` `[var(--space-*)]` `[var(--h-field-md)]` (all track `data-tune`) | a fixed Tailwind value (`rounded-lg`, `h-10`, `border`, `gap-12`, `mb-10`, `space-y-4`) where the value is *rhythm* and should follow `data-tune` |
