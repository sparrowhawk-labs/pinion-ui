...
**Ink — `primary` as a foreground (v0.17.0).** `primary` is a *surface* token (on Tonal themes it is a tone one step off the canvas; on `pop` it is a 1.6:1 yellow). Every theme block therefore also carries **`--pn-ink`**: primary itself when it already reads at ≥ 4.5:1 on both `base-100` and `base-200`, otherwise primary with its lightness pushed toward `base-content` until it reaches 5.0:1 (hue kept; `gen-themes.mjs` derives it for all 90 blocks, no category branch — 58 are identical to primary). The preset (`pinion-ui.css` → "ink") re-routes the **foreground** utilities of `primary` to `var(--pn-ink, var(--color-primary))` at build time: `text-primary[/N]`, `border-{,t,b,l,r,x,y}-primary[/N]`, `ring-primary[/N]`, `outline-` / `decoration-` / `fill-` / `stroke-` / `caret-primary`, and the translucent band `bg-primary/N` with **N ≤ 50**. Surfaces are untouched: `bg-primary`, `bg-primary/N` with N > 50, `text-primary-content`. The rule in one line: **write `text-primary` / `border-primary` / `bg-primary/10` as before — the preset guarantees they read on every theme; never reach for `accent` or a hex to "fix" a primary that looks invisible.** In hand-written CSS use `var(--pn-ink, var(--color-primary))` wherever the color is text, a line, a ring or a thin band (the bundled `doc-core.css` / `editor.css` / `sheet.css` / `data-grid.css` do). `text-ink-primary` / `bg-ink-primary` exist as explicit aliases (side effect of the theme key) — use **`bg-ink-primary` for an opaque primary fill that must read as a line or indicator** rather than a surface: progress / upload bar fills, stepper and timeline connectors, the sheet / data-grid fill-handle, and the range-slider track do (a `bg-primary` bar is invisible on a Tonal track). Checked checkbox / radio / toggle fills, the tooltip arrow and theme-swatch dots stay `bg-primary` (a surface with a `-content` mark on it). `ui:eject` freezes the same routing (`ink` column in `eject-table.json`, `bg-ink-primary` included).
...
pinion-ui ships **only original themes**. daisyUI's built-in themes are disabled (`themes: false`) — setting `data-theme="dracula"` or `"light"` does nothing. The lineup is 45 themes, each a **light/dark pair** (Tonal = tone-on-tone sets since v0.15.0: primary / secondary / accent share the base hue; primary is a surface tone one step off the canvas in both modes, so `bg-primary` surfaces flip with the mode and never invert. Such a primary is unreadable as *text* — which is why every theme block carries `--pn-ink` and the preset re-routes the foreground uses of `primary` to it; see **Ink** under the color vocabulary. Nothing to do on your side: keep writing `text-primary`):
