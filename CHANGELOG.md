...
### Added
- **Monochrome theme set — `carbon`, `glyph`, `noir`** (each a `<name>` / `<name>-dark` pair,
  category 美学系 (mood), lineup now 43 pairs). All three keep `primary` / `secondary` /
  `accent` in the *same hue family as the base* instead of a contrasting brand color, so
  they read as "characterless defaults": `carbon` = neutral charcoal greys (#0B0B0B ground,
  white primary), `glyph` = Nothing-style near-black × cold white (#050505 ground, blue-grey
  secondary), `noir` = smoky warm black × unbleached white (#0E0C0B ground, beige accent).
  Status colors come from the generator's per-mode OKLCH anchors (no `extra[]`).
  Generated by `gen-themes.mjs` from `src/resources/themes/lineup.json`; `eject-table.json`
  is not regenerated for these (same as `nonblok` in v0.9.x).
