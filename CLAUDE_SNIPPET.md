...
**Themes**: pinion-ui ships 36 original themes as `<name>` (light) / `<name>-dark` pairs — daisyUI's stock themes (`light`, `dark`, `dracula`, …) do **not** exist in the build. Default is `pinion`. When scaffolding a page, pick the theme whose domain matches the app (e.g. `payments`, `mood-monokai`, `atelier`) via **AGENTS.md → "Theme lineup & selection guide"** and set `<html data-theme="…" data-tune="…">`.
