...
| `width` | `string \| null` | `null` (= `'w-full max-w-[10rem]'`) | Override the outer wrapper's width utility — e.g. `'w-32'`, `'w-full'`. The default caps at 10rem so the row stays compact in cards / tables. |
...
- The default `width="w-full max-w-[10rem]"` caps the row so it doesn't stretch in flex layouts. Pass `width="w-full"` to disable the cap.
