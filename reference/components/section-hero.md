...
- Tune system ([`src/resources/css/tune.css`](../../src/resources/css/tune.css), bundled by the `pinion-ui.css` preset) — controls `--space-section` / `--space-section-inner`, which drive the hero's vertical rhythm.
