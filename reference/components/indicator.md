...
- **Single-character/icon-only badges render as a perfect circle, not an oval** (v0.4.3+): `indicator.blade.php` measures the rendered `badge` slot at render time (`trim(strip_tags(...))`, `mb_strlen(...) <= 1`) and passes a derived `circle` boolean into `IndicatorComposer::compose()` — the Composer itself never inspects markup, it just branches on that bool (fixed `h-5 w-5 p-0` instead of `h-5 min-w-[1.25rem] px-1.5`). A single digit ("3"), a single glyph ("!"), or an icon-only slot all get the circle; two-or-more characters ("12", "99+", "NEW") keep the standard pill shape with horizontal padding. This is separate from the `dot=true` case (already a fixed `h-3 w-3` circle) and doesn't require any prop from the caller — it's automatic based on slot content.
