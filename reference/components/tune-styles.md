...
- The presets shipped today are `default`, `sharp`, `soft`, `playful`, `corporate`, `brutal`, `elegant`, `bold`. (`pixel` and `tech` are listed in the README's preset table and are wired through `tune.css` in the CSS pipeline path — the Blade fallback currently emits the eight core presets.)
