...
| `inlineMd` | `bool` | `false` | DISPLAY-time rendering of a safe inline-markdown subset in text cells: `**bold**` / `*italic*` / `` `code` `` / `[text](url)`. The cell **editor still edits the raw source string** (round-trip lossless — for hosts whose canonical cell values are markdown, e.g. GFM-table-backed sheets). Values are HTML-escaped before markup (XSS-safe `x-html`); link hrefs limited to `http(s):`/`mailto:`/relative/`#`; underscore emphasis is deliberately unsupported (snake_case values must not italicize); `\*` `\_` etc. display as the bare character. |
